...
- Tabla limpia con links clickeables

Ejecución:
    pip install streamlit pandas requests
    streamlit run app_valeria_wallstreet_streamlit_v2_clickable.py
"""
...
def render_clickable_table(df: pd.DataFrame) -> None:
    table_df = df.copy()
    table_df["published_at"] = table_df["published_at"].dt.strftime("%Y-%m-%d %H:%M").fillna("")
    table_df["inserted_at"] = table_df["inserted_at"].dt.strftime("%Y-%m-%d %H:%M").fillna("")
    ...
    st.data_editor(
        ...
        disabled=True,
        column_config={
            "Abrir enlace": st.column_config.LinkColumn(
                "Abrir enlace",
                help="Haz clic para abrir el video o publicación",
                display_text="Abrir",
                validate=r"^https?://.*$",
            )
        },
...
def render_clickable_manual_links(df: pd.DataFrame) -> None:
    manual_view = df.copy()
    manual_view["added_at"] = manual_view["added_at"].dt.strftime("%Y-%m-%d %H:%M").fillna("")
    manual_view = manual_view.rename(
        columns={
            "title": "Título",
            "url": "Abrir enlace",
            "note": "Nota",
    ...
    st.data_editor(
        ...
        column_config={
            "Abrir enlace": st.column_config.LinkColumn(
                "Abrir enlace",
                help="Haz clic para abrir el link manual",
                display_text="Abrir",
                validate=r"^https?://.*$",
            )
        },
...
def main() -> None:
    ...
    st.title("📺 Valeria + Wall Street — V2 profesional")
    st.caption("Monitoreo, base de datos, filtros, reporte HTML y panel profesional en Streamlit.")
    ...
    with tab1:
        ...
        if filtered_df.empty:
            ...
            st.caption("Haz clic en la columna 'Abrir enlace' para abrir cada video directamente.")
            render_clickable_table(filtered_df)
            export_df = filtered_df.copy()
            export_df["published_at"] = export_df["published_at"].dt.strftime("%Y-%m-%d %H:%M").fillna("")
            export_df["inserted_at"] = export_df["inserted_at"].dt.strftime("%Y-%m-%d %H:%M").fillna("")
            st.download_button(
    ...
    with tab3:
        ...
        if manual_df.empty:
            ...
            st.caption("Haz clic en 'Abrir enlace' para abrir el recurso manual.")
            render_clickable_manual_links(manual_df)
            export_manual = manual_df.copy()
            export_manual["added_at"] = export_manual["added_at"].dt.strftime("%Y-%m-%d %H:%M").fillna("")
            st.download_button(
                "⬇️ Descargar CSV de links manuales",
                data=to_csv_bytes(export_manual),
    ...
    with tab4:
        ...
        st.write("Usa la búsqueda primero, luego aplica filtros, y después genera el reporte final.")

    st.markdown("---")
    st.markdown("### 🚀 Cómo ejecutar")
    st.code("pip install streamlit pandas requests\nstreamlit run app_valeria_wallstreet_streamlit_v2_clickable.py", language="bash")
    if not YOUTUBE_API_KEY:
        st.warning("No detecté YOUTUBE_API_KEY. El sistema intentará usar el fallback RSS, que puede ser menos estable que la API oficial.")
